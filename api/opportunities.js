...
function scoreOpportunity(o){
  ...
  if(/award|justification/.test(String(o.type||'').toLowerCase())){score-=40;risks.push('Appears to be an award/post-award notice')}
...
const seed=[{id:'seed-1',source:'SAM.gov',title:'Commercial Office Equipment & Supplies',agency:'Federal Agency',postedDate:'2026-08-09',deadline:'2026-08-21',location:'United States',country:'USA',type:'Solicitation',setAside:'Small Business — verify notice',description:'Demo opportunity shown only when the live SAM.gov feed is unavailable.',url:'https://sam.gov/opportunities',value:'Not disclosed',tags:['Product sourcing','Low specialization'],demo:true,active:true}];

function mapSam(x){
  const pop=x.placeOfPerformance||{};
  const country=pop.country?.name||pop.country?.code||'';
  const city=pop.city?.name||pop.city||'';
  const state=pop.state?.code||pop.state||'';
  const location=[city,state,country&&!['USA','US','United States','United States of America'].includes(country)?country:null].filter(Boolean).join(', ')||country||'See notice';
  ...
  return {id:x.noticeId,source:'SAM.gov',title:x.title||'Untitled opportunity',agency:x.fullParentPathName||[x.department,x.subTier,x.office].filter(Boolean).join(' · ')||'Federal agency',postedDate:x.postedDate,deadline:x.responseDeadLine||x.reponseDeadLine||null,location,country,type:x.type||'Opportunity',setAside,description:'BidScout screened this listing from SAM.gov metadata. Open the original solicitation for complete requirements and attachments.',url:`https://sam.gov/opp/${encodeURIComponent(x.noticeId)}/view`,value:amount?`$${Number(amount).toLocaleString('en-US')}`:'Not disclosed',tags:[x.naicsCode?`NAICS ${x.naicsCode}`:null,x.classificationCode?`PSC ${x.classificationCode}`:null].filter(Boolean),demo:false,active:String(x.active||'Yes').toLowerCase()==='yes'};
}

async function fetchSam(){
  const apiKey=process.env.SAM_API_KEY;
  if(!apiKey)return{items:[],error:'SAM_API_KEY not detected',code:'NO_KEY'};
  const now=Date.now();
  if(memoryCache.items&&now-memoryCache.updatedAt<MEMORY_TTL)return{items:memoryCache.items,fromCache:true};
  const today=new Date(),from=new Date(today);from.setDate(from.getDate()-7);
  const p=new URLSearchParams({api_key:apiKey,limit:'100',offset:'0',postedFrom:fmtDate(from),postedTo:fmtDate(today)});
  const r=await fetch(`https://api.sam.gov/opportunities/v2/search?${p}`,{headers:{accept:'application/json'}});
  if(!r.ok){
    const code=r.status===429?'RATE_LIMITED':`HTTP_${r.status}`;
    if(memoryCache.items)return{items:memoryCache.items,fromCache:true,stale:true,error:`SAM.gov API returned ${r.status}`,code};
    return{items:[],error:`SAM.gov API returned ${r.status}`,code};
  }
  const j=await r.json();
  const items=(j.opportunitiesData||[]).map(mapSam).filter(o=>o.active!==false).map(o=>({...o,...scoreOpportunity(o)})).sort((a,b)=>b.score-a.score);
  if(items.length){memoryCache={items,updatedAt:Date.now()}}
  return{items};
}

module.exports=async function handler(req,res){
  const result=await fetchSam();
  const hasLive=result.items&&result.items.length>0;
  if(hasLive){
  ...
  const items=hasLive?result.items:seed.map(o=>({...o,...scoreOpportunity(o)}));
  const mode=hasLive?(result.stale?'stale':'live'):(result.code==='RATE_LIMITED'?'rate_limited':'demo');
  res.status(200).json({mode,error:result.error||null,code:result.code||null,updatedAt:new Date().toISOString(),items});
}
